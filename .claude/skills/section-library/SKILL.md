...
For Owen to browse visually, open `index.html` in a browser, or point him to the site's live Vercel URL (it's the homepage) — it renders every section as a live scaled thumbnail with type/feel filter chips and search, regenerated via `python3 scripts/build_gallery.py`.
